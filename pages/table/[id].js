...
export default function Table() {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      try {
        ...
        // Auto-refresh only once when user connects
  ...
  useEffect(() => {
    ...
    const handleScroll = () => { if (el) el.scrollLeft > 0; };
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      if (!response.ok || !order.id) throw new Error(order.error || 'Order failed');
  ...
  const updateOrder = async () => {
    ...
    try {
      ...
      const order = await response.json();
      if (!response.ok || !order.id) throw new Error(order.error || 'Update failed');
      setCart([]);
      router.replace(`/order/${order.id}`);
    } catch (err) {
  ...
  return (
    <section className="min-h-screen bg-gray-50 p-4 relative">
      {/* ❌ Show modal if not on café Wi-Fi */}
      {checking ? (
        ...
        <div className="fixed inset-0 bg-white z-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center max-w-sm mx-auto">
            ...
            <p className="text-gray-700 mb-4">You must be connected to the café Wi-Fi to access the menu.</p>
            <button
              className="bg-blue-600 text-white px-4 py-2 rounded mb-2"
              onClick={() => {
                // Android Wi-Fi settings intent (works only in WebView or Android Chrome)
